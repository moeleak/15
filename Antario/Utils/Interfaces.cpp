...
namespace interfaces
{
    ...
    void Init()
    {
...
#endif
        g_pClientDll        = CaptureInterface<IBaseClientDLL>(XORSTR("client.dll"), XORSTR("VClient018"));
        ...
        g_pEntityList       = CaptureInterface<IClientEntityList>(XORSTR("client.dll"), XORSTR("VClientEntityList003"));
        g_pPrediction       = CaptureInterface<CPrediction>(XORSTR("client.dll"), XORSTR("VClientPrediction001"));
        g_pMovement         = CaptureInterface<IGameMovement>(XORSTR("client.dll"), XORSTR("GameMovement001"));
        ...
        g_pViewRender       = **reinterpret_cast<IViewRender***>(Utils::FindSignature(XORSTR("client.dll"), XORSTR("A1 ? ? ? ? B9 ? ? ? ? C7 05 ? ? ? ? ? ? ? ? FF 10")) + 1);
        ...
        g_pWeaponSystem     = *reinterpret_cast<IWeaponSystem**>(Utils::FindSignature(XORSTR("client.dll"), XORSTR("FF 50 04 83 BE ? ? ? ? ? 74 0A")) - 4);
        ...
        g_pPlayerResource           = *reinterpret_cast<C_CSPlayerResource**>(*(uintptr_t**)(Utils::FindSignature(XORSTR("client.dll"), XORSTR("A1 ? ? ? ? 57 85 C0 74 08")) + 1));
#ifndef _DEBUG
        VMProtectEnd();
#endif 
    }
